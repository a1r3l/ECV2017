...
function Canvas(){
	...
	this.wallthre= null;
	this.wallfour = null;
	this.wallDataImgOne = null;
	this.wallDataImgTwo = null;	
};
...
Canvas.prototype.createPlane = function(x,y,z,name){
	...
	var texture = this.textureLoader.load('public/imgs/grass.jpg');
...
Canvas.prototype.createAcera = function(name){
	...
	var texture = this.textureLoader.load('public/imgs/baldosa.jpg');
	...
	var lateralTexture = this.textureLoader.load('public/imgs/baldosa.jpg');
...
Canvas.prototype.createBuilding = function(x,y,z,name){
	...
	mtlLoader.load('public/obj/build2/building.mtl', function(materials){
		materials.preload();
		var objLoader = new THREE.OBJLoader(manager);
		objLoader.setMaterials(materials);
		objLoader.load('public/obj/build2/building.obj',function(object){
...
Canvas.prototype.createOBJ = function(x,y,z,name){
	var that = this;
	var manager = new THREE.LoadingManager();
	var texture = this.textureLoader.load('public/obj/build10/Build10.jpg')
	var objLoader = new THREE.OBJLoader(manager);
    var material = new THREE.MeshBasicMaterial({map:texture});
    objLoader.load('public/obj/build10/Build10_obj.obj', function (obj) {
...
Canvas.prototype.createStop = function() {
	...
	var manager = new THREE.LoadingManager();
	var mtlLoader = new THREE.MTLLoader();
	mtlLoader.load('public/obj/stop/stop.mtl', function(materials){
		materials.preload();

		var objLoader = new THREE.OBJLoader(manager);
		objLoader.setMaterials(materials);
		objLoader.load('public/obj/stop/stop.obj',function(object){
...
Canvas.prototype.createLamp = function(x,y,z,name) {
	...
	var manager = new THREE.LoadingManager();
	var mtlLoader = new THREE.MTLLoader();
	mtlLoader.load('public/obj/lamp/StreetLamp.mtl', function(materials){
		materials.preload();

		var objLoader = new THREE.OBJLoader(manager);
		objLoader.setMaterials(materials);
		objLoader.load('public/obj/lamp/StreetLamp.obj',function(object){
			object.scale.set(4,6,4);
			object.position.set(x,y,z)
...
Canvas.prototype.createTree = function(x,y,z) {
	...
	var manager = new THREE.LoadingManager();
	var mtlLoader = new THREE.MTLLoader();
	mtlLoader.load('public/obj/tree/tree.mtl', function(materials){
		materials.preload();

		var objLoader = new THREE.OBJLoader(manager);
		objLoader.setMaterials(materials);
		objLoader.load('public/obj/tree/tree.obj',function(object){
			object.scale.set(4,6,4);
			object.position.set(x,y,z)
			that.scene.add(object);			
		});
...
Canvas.prototype.createStreet = function(name){
	...
	var texture = this.textureLoader.load('public/imgs/carretera.jpg');
	texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
	...
	var cornerTexture = this.textureLoader.load('public/imgs/esq_drch.jpg');
	...
	var cornerTextDer = this.textureLoader.load('public/imgs/esq_izq.jpg');
	...
	var texture2 = this.textureLoader.load('public/imgs/carretera_up.jpg');
...
Canvas.prototype.createSceneWalls = function(name){
	...
	var texture = this.textureLoader.load('public/imgs/wall.3.jpg');
	...
	leftWall.name = name;
	//Entrance
	var gateGeo = new THREE.PlaneGeometry(128,64);
	var gateTexture = this.textureLoader.load('public/imgs/entrance.png');
	var gateMat = new THREE.MeshPhongMaterial({map:gateTexture,transparent:true});
	var gate = new THREE.Mesh(gateGeo,gateMat);
	gate.name = "Gate";
	gate.position.set(0,35,0);
...
Canvas.prototype.createSkybox = function(name){
	var materialArray = [];
	materialArray.push(new THREE.MeshBasicMaterial( { map: this.textureLoader.load( 'public/imgs/skybox/posx.png' ) }));
	materialArray.push(new THREE.MeshBasicMaterial( { map: this.textureLoader.load( 'public/imgs/skybox/negx.png' ) }));
	materialArray.push(new THREE.MeshBasicMaterial( { map: this.textureLoader.load( 'public/imgs/skybox/posy.png' ) }));
	materialArray.push(new THREE.MeshBasicMaterial( { map: this.textureLoader.load( 'public/imgs/skybox/negy.png' ) }));
	materialArray.push(new THREE.MeshBasicMaterial( { map: this.textureLoader.load( 'public/imgs/skybox/posz.png' ) }));
	materialArray.push(new THREE.MeshBasicMaterial( { map: this.textureLoader.load( 'public/imgs/skybox/negz.png' ) }));
	for (var i = 0; i < 6; i++)
...
Canvas.prototype.createSideBuildings = function(x,y,z,name) {
	...
	var texture = this.textureLoader.load('public/imgs/sidebuild.png');
...
Canvas.prototype.setEventsListeners = function(){
	...
	this.renderer.domElement.addEventListener('mousedown', function(e){
		...
		var myClientY = e.clientY - 50;
		mouse.x = (myClientX / that.renderer.domElement.width) * 2 - 1;
    	mouse.y = -(myClientY / that.renderer.domElement.height) * 2 + 1;
		paint = true;
		that.drawOnPlane();
...
Canvas.prototype.drawOnPlane = function(){
	...
	for (var i = 0 ; i < intersects.length; i++){
		if(intersects[i].object.name == "planoone"){
			...
			//Posició al canvas a pintar
			var canvasX = xPos + 320;
			var canvasY = 64 - yPos; 
			//Variables per enviar amb comunicació
			var color = intersects[i].object.wall.twoDcanvas.context.fillStyle;
			var size = intersects[i].object.wall.twoDcanvas.context.lineWidth;
			var canvasToPaint = intersects[i].object.wall.twoDcanvas.name;

			var ObjectTosend = {
				type : "onPaint",
				canvasName: canvasToPaint,
...
Canvas.prototype.drawOnMessage = function (canvas,position,color,size){
	var that = this;
	var twodcanvas = null;
	
	if(canvas == "twodCanvasone"){
		twodcanvas = that.scene.getObjectByName("planoone").wall.twoDcanvas;
		twodcanvas.drawOnMesage(twodcanvas,position,color,size);
	}

	else if(canvas == "twodCanvastwo"){
		twodcanvas = that.scene.getObjectByName("planotwo").wall.twoDcanvas;
		twodcanvas.drawOnMesage(twodcanvas,position,color,size);
	}
}
...
Canvas.prototype.sendCanvasState = function(){

	var canvasOne = this.scene.getObjectByName("planoone").wall.twoDcanvas;
	var canvasTwo = this.scene.getObjectByName("planotwo").wall.twoDcanvas;

	var canvasOnedata = canvasOne.canvas.toDataURL();
	var canvasTwodata = canvasTwo.canvas.toDataURL();

	var ObjectTosend = {
		type:"canvasState",
		dataImageOne: canvasOnedata,
		dataImageTwo:canvasTwodata
	}

	ws.send(JSON.stringify(ObjectTosend));
}

Canvas.prototype.setUpdatedState = function(miss) {

	this.wallDataImgOne = miss.dataImageOne;
	this.wallDataImgTwo = miss.dataImageTwo;
	this.setUpdated();
};

Canvas.prototype.setUpdated = function(){

	var canvasOne = this.scene.getObjectByName("planoone").wall.twoDcanvas;
	var canvasTwo = this.scene.getObjectByName("planotwo").wall.twoDcanvas;
	canvasOne.aplyUpdate(this.wallDataImgOne);
	canvasTwo.aplyUpdate(this.wallDataImgTwo);
